LazySloth Core version 0.14.2 is now available from:

  <https://download.lazysloth.org/lazysloth-0.14.2.0/>
...
LazySloth Core now supports P2SH addresses beginning with M on mainnet and Q on testnet.
...
This only affects users that have explicitly enabled UPnP through the GUI setting or through the -upnp option, as since the last UPnP vulnerability (in LazySloth Core 0.10.4) it has been disabled by default.
...
Testnet3 has been deprecated and replaced with Testnet4. The server port has been changed to 34644 however the RPC port remains
the same (34643).

Testnet faucets can be located at:
- http://testnet.lazyslothtools.com
...
LazySloth Core has supported automatically pruning the blockchain since 0.13.2. Pruning
...
When LazySloth Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `lazysloth-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`lazysloth-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/lazysloth-cli -named help command="help"
    src/lazysloth-cli -named getblockhash height=0
    src/lazysloth-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/lazysloth-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was disabled and deprecated in LazySloth Core 0.10.4 and removed in 0.13.2. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of LazySloth Core. This was added in conjunction with
...
here, originally written for Bitcoin but can also be used for LazySloth:
