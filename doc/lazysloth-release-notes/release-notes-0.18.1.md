LazySloth Core version 0.18.1 is now available from:

  <https://download.lazysloth.org/lazysloth-0.18.1/>
...
`/Applications/LazySloth-Qt` (on Mac) or `lazyslothd`/`lazysloth-qt` (on
Linux).
...
LazySloth Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use LazySloth Core on unsupported systems.
...
10.10. Additionally, LazySloth Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:24643:24643` (this is an extra `:24643` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@lazysloth.org) the LazySloth Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 LZSL),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in LZSL per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/lazysloth-conf.md)
  about the `lazysloth.conf` file describes how to use it to configure
  LazySloth Core.

- A new document introduces LazySloth Core's BIP174 [Partially-Signed
  LazySloth Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent LazySloth-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  LazySloth Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when LazySloth Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of LazySloth Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart lazyslothd with the `-deprecatedrpc=generate`
...
- A new `lazysloth-wallet` tool is now distributed alongside LazySloth
...
This section describes planned changes to LazySloth Core that may affect
other LazySloth software and services.

- Since version 0.16.0, LazySloth Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with LazySloth Core 0.20 (expected about
  a year after 0.18), LazySloth Core will default to native segwit
  ...
  to bech32 addresses, and if the LazySloth Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in LazySloth Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any LazySloth Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, LazySloth Core
...
- A sub-project of LazySloth Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with LazySloth Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to LazySloth Core's own implementation, although entropy
  gathered by LazySloth Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves LazySloth Core a
...
- On macOS, LazySloth Core now opts out of application CPU throttling
...
- #24643 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
