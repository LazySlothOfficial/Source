...
The LazySloth Core daemon can be configured to act as a trusted "border
router", implementing the lazysloth wire protocol and relay, making
...
The ZeroMQ feature in LazySloth Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building lazyslothd:
...
For instance:
    ...
    $ lazyslothd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/lazyslothd.tx.raw \
...
These options can also be provided in lazysloth.conf.
...
From the perspective of lazyslothd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into lazyslothd directly. Furthermore, no information is
...
using. LazySlothd appends an up-counting sequence number to each
