...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " lazysloth-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  lazysloth-tx [options] <hex-tx> [commands]  Update hex-encoded lazysloth transaction\n" +
            "or:     lazysloth-tx [options] -create [commands]   Create hex-encoded lazysloth transaction\n" +
            "\n";
