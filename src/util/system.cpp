...
const char * const BITCOIN_CONF_FILENAME = "lazysloth.conf";
...
#else
    const char* pszModule = "lazysloth";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LazySloth";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/LazySloth";
#else
    // Unix-like
    return pathRet / ".lazysloth";
#endif
#endif
}
