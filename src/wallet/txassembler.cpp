    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional LZSL outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes LZSL and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::LZSL_TO_LZSL) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        if (SelectCoins(new_tx, nTargetValue, params_pegin)) {
            return std::any_of(new_tx.selected_coins.cbegin(), new_tx.selected_coins.cend(), is_lzsl);
        }
    } else {
        // First try to construct a LZSL-to-LZSL transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::LZSL_ONLY;
        ...
        // If LZSL-to-LZSL fails, create a peg-out transaction
